...
type Repo struct {
	...
	Forks   int      // fork 数
	Add     int      // 周期内新增
	BuiltBy []string // 贡献值 avatar img 连接
}
...
func getTrending(language, dateRange string) ([]*Repo, error) {
	...
	doc.Find(".Box .Box-row").Each(func(i int, s *goquery.Selection) {
		...
		spanSel := s.Find("dev>span")
		var langIndex, addStartIndex, builtByIndex int
		...
		parts := strings.SplitN(strings.TrimSpace(spanSel.Eq(addStartIndex).Text()), " ", 2)
		addStar, _ := strconv.Atoi(parts[0])
		repo.Add = addStar
		...
		repo.Start = star
		forkStr := strings.TrimSpace(anchorSel.Eq(-2).Text())
